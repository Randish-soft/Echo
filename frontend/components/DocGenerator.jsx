...
export default function DocGenerator({ selectedRepo, onDocumentGenerated, onBack }) {
  ...
  const generatePdfPreview = async () => {
    try {
      // Simple HTML preview for now - you can replace with actual PDF generation
      const htmlContent = `
        <!DOCTYPE html>
        <html>
          <head>
          ...
          <div className="flex-1 p-4 overflow-hidden">
            {isEditing ? (
              <textarea
                ...
                className="w-full h-full px-4 py-3 bg-slate-800 border border-slate-600 rounded-lg 
                         text-white font-mono text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Start editing your documentation here..."
              />
            ) : (
              <div className="w-full h-full px-4 py-3 bg-slate-800 border border-slate-600 rounded-lg 
                            text-white font-mono text-sm whitespace-pre-wrap overflow-auto">
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex items-center justify-between p-4 bg-slate-800 border-b border-slate-700">
            ...
            <button
              ...
              disabled={!pdfUrl}
              className="px-3 py-1 bg-red-600 hover:bg-red-700 disabled:bg-red-600/50 
